...
#%% Data 
files_all = np.sort(os.listdir(fp+'Jnew/'))
...
for fil in zip(files_all, tqdm(colours)):
    data = Table.read(fp+f'Jnew/{fil[0]}', format='fits')
    ...
    for num in nums:
        sigma = intp(num)
        np.save(f'./data/detection_curve_j/j{int(num)}_detect_lim.npy', sigma)
...
plt.savefig('Fig_3.3_Jband.pdf', bbox_inches="tight")
plt.show()

np.save('./data/jlowflx.npy', flow)
np.save('./data/jhighflx.npy', fhigh)    