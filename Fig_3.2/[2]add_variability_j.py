...
#%%Getting the flux from the data
    ...
    jflux_start = colnames.index('J_flux_05B')
    jflux_end = colnames.index('J_fluxerr_12B') + 1
    
    chi2j_org = np.array(data['chi2_j'])
    sigj_org = np.array(data['sigma j'])
    
    chi2j_final = np.empty((101, 0), float)
    sigj_final = np.empty((101, 0), float)
    varj_final = np.empty((101, 0), float)
    
    for i in tqdm(ID):
        rw = list(data[np.where(int(i)==ID)[0][0]])
        
        flux_all = rw[jflux_start:jflux_end] 
        ...
        # fig, ax = plt.subplots()
        # ax.errorbar(sems, flux_norm, fluxerr_norm, color='dodgerblue', ecolor='lightgrey', marker='o', ls='none')
        # ax.set(xlabel='Semester', ylabel='J-Band Flux (Normalised)')
    ...
    #%%Generating likelihood curve maximum sigma
        ...
        # ax.plot(sigma_rng, l/np.max(l), color='dodgerblue', ls='--')
        ...
        for sigq in sigma_rng:
            ...
            # ax.errorbar(sems, flux_new, fluxerr_norm, color='dodgerblue', ecolor='lightgrey', marker='o', ls='none')
            ...
            # ax.plot(sigma_rng, l_new/np.max(l_new), color='dodgerblue', ls='--')
        ...
        chi2j_final = np.column_stack((chi2j_final, chi2_newsave))
        sigj_final = np.column_stack((sigj_final, max_sigma_newsave))
        varj_final = np.column_stack((varj_final, var_newsave))
    
        #%% Adding columns to data and saving file
    
    col_sigq = np.char.add(np.array(['sigq']).astype(str), sigma_rng.astype(str))
    col_chi2j = np.char.add(np.array(['chi2_j']).astype(str), sigma_rng.astype(str))
    col_var_j = np.char.add(np.array(['Var_j']).astype(str), sigma_rng.astype(str))
    
    final_names = np.concatenate((col_chi2j, col_sigq, col_var_j), axis=0)
    
    data_calc =  np.vstack((chi2j_final, sigj_final, varj_final)).T
    
    print("Shape of data:", len(data))  # should be 3001
    print("Shape of data_calc:", data_calc.shape)  # should be (3001, 303)
