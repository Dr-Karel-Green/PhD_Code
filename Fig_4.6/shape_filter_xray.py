...
data_reg = Table.read('Data/notxray.fits', format='fits') 
...
data = Table.read('Data/xrayonly.fits', format='fits') 
...
ax.hist(shape, bins=20, histtype='step', color='deepskyblue', range=(0,1), density=True, label='Variable only')
...
for subfig in figs.flatten():
    ...
    ax.scatter(Mj[mask], mass[mask], marker='o', fc='deepskyblue', edgecolor='navy', label='X-ray')
    ax.scatter(Mj[mask_star], mass[mask_star], marker='*', fc='deepskyblue', edgecolor='navy', linewidth=1)
...
xray_in_contour=0 #After loop printing this value gives the number of AGN that was in the contour
...
for subfig in figs.flatten():
    ...
    xray_in_contour += np.nansum(filt) + np.nansum(filt_star)
    xv_in_contour += np.nansum(filtxv) + np.nansum(filtxv_star)
    
    ax.scatter(Mj_inact[mask_inact], mass_inact[mask_inact], marker='.', s=5, color='lightgrey')
    ax.scatter(Mj[mask][filt], mass[mask][filt], marker='o', fc='deepskyblue', edgecolor='navy', label='X-ray')
    ax.scatter(Mj[mask_star][filt_star], mass[mask_star][filt_star], marker='*', fc='deepskyblue', edgecolor='navy', linewidth=1)
...
#%%
ID = np.array(data['ID_UDS'])
IDxv = np.array(xv['DR11_ID'])

xray_keep = np.array([]) #Variable AGN that have all the mass and mag filters but are also within 2-sigma of the regular galaxies
xv_keep = np.array([])
...
for subfig in figs.flatten():
    ...
    ax.scatter(Mj_inact[mask_inact], mass_inact[mask_inact], marker='.', s=5, color='lightgrey')
    ax.scatter(Mj[mask][filt], mass[mask][filt], marker='o', fc='deepskyblue', edgecolor='navy', label='X-ray')
    ax.scatter(Mjxv[maskxv][filtxv], massxv[maskxv][filtxv], marker='o', fc='mediumorchid', edgecolor='darkviolet', label='X-ray & X-ray')
    ax.set(xlabel='J - Band Absolute Magnitude', ylabel=r'Stellar Mass [$log(M_{*}$/$M_{\odot}$)]', xlim=(-27, -19), ylim=(8.5, 11.65), title=fr'{n[i]} $<$ z $\leq$ {m[i]}')
    ax.invert_xaxis()
    ...
    ID_keep = ID[bin_filt]
    xray_keep = np.append(xray_keep, ID_keep)
...
#%%
np.save('Data/xray_id.npy', xray_keep) #The variable AGN that lie within the 2sigma (95%) locus of the regular galaxies
